...
HOOK_GENERATION_INSTRUCTIONS = (
    ...
    "contradiction stated as a sentence. Never "
...
TRIPLE_HOOK_JUDGE_INSTRUCTIONS = (
    ...
    "sentence semantically PERFORMS it — judge its rhetorical function, not its vocabulary (a curiosity gap may be a "
    "statement that opens an unresolved contradiction, with no question word); the strategy's name earns nothing by "
    "itself. natural_language: would a native speaker say exactly this sentence aloud? Score low for translated-sounding "
    "or template-like wording, a missing subject or complement, a comparison without saying what it is compared with, "
    "and sentences that parse but that nobody would say; a slightly simpler but awkward hook loses to an equally clear "
    "natural one. Hard rules: a "
